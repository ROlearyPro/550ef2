...
function NodeDetail({ BlueprintData }: any) {
    ...
    useEffect(() => {
        // If we don't have a node at any point, return
        if (!node) return;
        ...
        // if we haven't clicked an empty form to prefill it, return
        if (!form) return;
    ...
    // Sets some default global data to use
    ...
    const defaultOptions = globalDefaultKeys
        ...
        // Makes sure you can still type in a field if clicking it causes a modal to open 
    ...
    // Renders the form area for user to see.
    return (
